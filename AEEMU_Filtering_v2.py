...
from AEEMU_Filtering import (
    ...
    # Models
    MatrixFactorization, BayesianNCF, RecVAE, SASRec, LightGCN, EASERecommender,
    # SOTA baselines
    MBRCCBaseline, DualChannelGCN,
    # Meta-network
    MetaNeuralNetworkWithFilters, MetaNetworkTrainerWithFilters,
    # Ensemble
    SimplePerformanceWeightedEnsemble, UniformAverageEnsemble,
    # Data loaders
    load_movielens_100k, load_movielens_1m,
    load_amazon_digital_music, load_book_crossing, load_jester_jokes,
    # Utilities
...
def compare_with_sota_full(
    ...
    baseline_factories = {
        ...
        'EASE': lambda: EASERecommender(lam=500.0),   # Steck 2019 — strong linear SOTA
    ...
    for fold_idx, (train_index, test_index) in enumerate(kf.split(df)):
        ...
        if len(trained_baselines) >= 2:
            print(f"   🔧 Evaluating Simple Weighted Ensemble + Uniform Average...")
            try:
                ...
                # Uniform average: equal weight for all base models (clearest baseline)
                uniform_ens = UniformAverageEnsemble(list(trained_baselines.keys()))
                ...
                user_preds_uniform = defaultdict(list)
                ...
                for _, row in test_df.iterrows():
                    ...
                    if u < train_matrix.shape[0] and i < train_matrix.shape[1]:
                        ...
                        upred = uniform_ens.predict(base_preds)
                        simple_preds.append(pred)
                        uniform_preds.append(upred)
                        true_ratings.append(row['rating'])
                        user_preds_simple[u].append((i, pred, row['rating']))
                        user_preds_uniform[u].append((i, upred, row['rating']))
                ...
                if uniform_preds:
                    u_rmse = np.sqrt(mean_squared_error(true_ratings, uniform_preds))
                    u_mae = mean_absolute_error(true_ratings, uniform_preds)
                    all_fold_results['UniformAvgEns']['rmse'].append(u_rmse)
                    all_fold_results['UniformAvgEns']['mae'].append(u_mae)

                    filtered_u = {u: p for u, p in user_preds_uniform.items() if len(p) >= 5}
                    if filtered_u:
                        rank_m_u = compute_ranking_metrics(filtered_u, k_values=[5, 10, 20])
                        for mk, mv in rank_m_u.items():
                            all_fold_results['UniformAvgEns'][mk].append(mv)

                    print(f"      ✅ UniformAvgEns: RMSE={u_rmse:.4f}, MAE={u_mae:.4f}")

            except Exception as e:
                print(f"      ❌ SimpleWtdEns/UniformAvgEns failed: {e}")
    ...
    if aeemu_methods:
        ...
        if len(best_aeemu_folds) >= 5:
            ...
            for method_name, method_data in sorted_methods:
                ...
                # Wilcoxon signed-rank test (one-sided: H1 = other method RMSE > AEEMU RMSE)
                # zero_method='zsplit' avoids p=0.5 artefact with tied differences.
                try:
                    diffs = other_arr - baseline_arr  # positive → other method is worse
                    if np.any(diffs != 0):
                        _, w_p = wilcoxon(diffs, alternative='greater', zero_method='zsplit')
                    else:
                        w_p = 1.0
                except:
                    w_p = 1.0
...
def _run_aeemu_single_fold(
    ...
    # Train meta-network (multiple epochs for convergence)
    # NOTE: 5 epochs caused overfitting on the small 15% val split (higher RMSE than 1 epoch).
    # 3 epochs is a better trade-off confirmed by comparing v1 (1 epoch, RMSE~0.86) vs v2 (5 epochs, RMSE~0.94).
    n_meta_epochs = 3
...
def compare_with_sota_full_multi(n_folds: int = 10):
    ...
    datasets = ['ml-100k', 'ml-1m', 'amazon-music', 'book-crossing', 'jester']
    all_results = {}
...
def _generate_cross_dataset_table(all_results: dict, n_folds: int):
    ...
    # Use display names stored in results (e.g. "Amazon Digital Music", "Book-Crossing")
    # Fall back to slug if not available
    display_name_map = {ds: (all_results[ds].get('dataset_name', ds) if all_results[ds] else ds)
    ...
    # Header row 1: dataset display names
    header1 = r'\textbf{Method}'
    for ds in dataset_names:
        dn = display_name_map[ds].replace('_', r'\_')
        header1 += r' & \multicolumn{2}{c}{\textbf{' + dn + r'}}'
    header1 += r' \\'
    ...
    # Order: individual baselines first, then UniformAvgEns, then SimpleWtdEns, then AEEMU
    individual = sorted([m for m in all_methods
                        if not m.startswith('AEEMU_') and m not in ('SimpleWtdEns', 'UniformAvgEns')])
    aeemu = sorted([m for m in all_methods if m.startswith('AEEMU_')])
    method_order = individual + ['UniformAvgEns', 'SimpleWtdEns'] + aeemu
    ...
    for method in method_order:
        ...
        if method == 'UniformAvgEns':
            lines.append(r'\midrule')

        if method == 'SimpleWtdEns':
            pass  # already separated above by UniformAvgEns midrule
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--sota-full-multi',
        action='store_true',
        help="[V2] Run --sota-full on all 5 datasets (ml-100k, ml-1m, amazon-music, book-crossing, jester)."
    )
    parser.add_argument(
        '--best-config',
        type=str,
    ...
    parser.add_argument('--filter-ordering', action='store_true',
                        help="[V2] Ablation on filter ordering (sequential/reversed/parallel). "
    ...
    if args.sota_full_multi:
        ...
        # Filter ordering ablation — Reviewer #3 Comment 2
