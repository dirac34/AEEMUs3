...
class RecVAE(BaseRecommender, nn.Module):
    ...
    def predict(self, user_id: int, item_id: int) -> float:
        ...
        if user_id >= self.recon_matrix.shape[0] or item_id >= self.recon_matrix.shape[1]:
            return float(np.clip(self.global_mean, 1.0, 5.0))
        raw = float(self.recon_matrix[user_id, item_id])
        # Sigmoid decoder pushes unobserved (test) items near 0 → clipped to 1.0.
        # Fall back to global mean so test predictions are not systematically biased low.
        if raw < 1.0:
            return float(np.clip(self.global_mean, 1.0, 5.0))
...
# Steck, H. (2019). WWW 2019. doi:10.1145/3308558.3313710
# Closed-form item-item similarity with L2 regularisation (no NN).
# Competitive with deep models; serves as a strong linear baseline.
# =================================================================

class EASERecommender(BaseRecommender):
    """
    Embarrassingly Shallow Autoencoder (EASE) for explicit rating prediction.

    Closed-form solution: B = I - P · diag(1/diag(P))
    where P = (X^T X + λI)^{-1}, X is the user–item rating matrix.
    Predictions are the off-diagonal entries of X · B, clipped to [1, 5].

    Reference: Steck (2019), WWW. doi:10.1145/3308558.3313710
    """
    ...
    def __init__(self, lam: float = 500.0):
        ...
        self.rating_min: float = 1.0
        self.rating_max: float = 5.0
        self._train_matrix: Optional[np.ndarray] = None

    def fit(self, train_matrix: np.ndarray, train_df: pd.DataFrame) -> None:
        self.global_mean = float(train_df['rating'].mean())
        self.rating_min = float(train_df['rating'].min())
        self.rating_max = float(train_df['rating'].max())
        self._train_matrix = train_matrix.copy()

        # X: users × items (float64 for numerical stability)
        X = train_matrix.astype(np.float64)
        G = X.T @ X                                   # n_items × n_items
        diag_idx = np.arange(G.shape[0])
        G[diag_idx, diag_idx] += self.lam             # regularise

        P = np.linalg.inv(G)                          # (X^T X + λI)^{-1}
        # EASE closed-form: zero out diagonal, normalise columns
        self.B = P / (-np.diag(P))                    # element-wise column div
        self.B[diag_idx, diag_idx] = 0.0              # enforce zero self-loop

    def predict(self, user_id: int, item_id: int) -> float:
        if self.B is None or self._train_matrix is None:
            return self.global_mean
        if user_id >= self._train_matrix.shape[0] or item_id >= self._train_matrix.shape[1]:
            return self.global_mean
        user_row = self._train_matrix[user_id].astype(np.float64)   # shape (n_items,)
        pred = float(user_row @ self.B[:, item_id])
        # Fall back to global mean for unseen users (all-zero rows)
        if pred == 0.0:
    ...
    def get_embeddings(self) -> Tuple[np.ndarray, np.ndarray]:
        # EASE has no latent user/item embeddings; return empty arrays
        n_users = self._train_matrix.shape[0] if self._train_matrix is not None else 0
        n_items = self._train_matrix.shape[1] if self._train_matrix is not None else 0
        return np.zeros((n_users, 1)), np.zeros((n_items, 1))
...
class MetaNetworkTrainerCorrected:
    ...
    def compute_performance_guided_loss(self, ensemble_weights, ensemble_pred, true_ratings):
        ...
        Loss function (R#4.13 — clarified):
            L = L_pred + α·L_align - β·H(w) + γ·L_rank + δ·L_best
        where:
            L_pred  = MSE(r̂_ui, r_ui)                            — prediction MSE
            L_align = ||w - w*||²                                  — alignment with optimal weights
                      (w* = inverse-RMSE normalised, computed on validation folds)
            H(w)    = -Σ_k w_k log(w_k)                          — entropy regularizer (prevents collapse)
            L_rank  = max(0, w_worse - w_better + margin)         — pairwise margin ranking loss
            L_best  = max(0, min_w - w_NCF)                       — minimum-weight constraint for NCF

        Note: there is no "accuracy" term; all objectives are defined on RMSE/MSE,
        consistent with the regression setting (addresses Reviewer #4 Comment 13).
        Hyperparameters: α=5.0, β=0.05, γ=10.0, δ=8.0, margin=0.1, min_w=0.35
        """
...
class UniformAverageEnsemble:
    """
    Uniform (unweighted) average ensemble — explicit 'simple average' baseline.

    Addresses Reviewer #4 Comment 16 and Reviewer #6 Comment 2:
    The paper must define the baseline ensemble recipe precisely.  This class
    implements the **uniform-average baseline**: all K base models get equal
    weight 1/K, regardless of their individual performance.  It is clearly
    distinct from:
      - Individual base models (MF, NCF, SASRec, LightGCN)
      - Performance-weighted ensemble (inverse-RMSE weights)
      - AEEMU (instance-adaptive meta-network weights)

    Having all three ensemble levels in the SOTA table lets readers isolate
    how much gain comes from (a) ensembling at all, (b) performance weighting,
    and (c) instance-adaptive weighting + signal-processing filters.
    """

    def __init__(self, model_names: List[str]):
        self.model_names = model_names
        self.weight = 1.0 / max(len(model_names), 1)

    def predict(self, predictions: Dict[str, float]) -> float:
        values = [predictions[n] for n in self.model_names if n in predictions]
        if not values:
            return 3.0
        return float(np.clip(np.mean(values), 1.0, 5.0))

    def get_weights(self) -> Dict[str, float]:
        return {name: self.weight for name in self.model_names}
...
def load_movielens_1m() -> Tuple[pd.DataFrame, np.ndarray]:
    ...
    # Filter active users (>= 10 ratings) and popular items (>= 5 ratings)
    print(f"📊 ML-1M initial: {len(df)} ratings from {df['user_id'].nunique()} users, {df['item_id'].nunique()} items")
    user_counts = df['user_id'].value_counts()
    df = df[df['user_id'].isin(user_counts[user_counts >= 10].index)].copy()
    item_counts = df['item_id'].value_counts()
    df = df[df['item_id'].isin(item_counts[item_counts >= 5].index)].copy()

    # Subsample to ~200K ratings (same cap as Amazon Music)
    if len(df) > 200000:
        df = df.sample(n=200000, random_state=42).reset_index(drop=True)
    print(f"📊 ML-1M after filtering/sampling: {len(df)} ratings")

    # Create compact user and item mappings
    user_mapping = {old: new for new, old in enumerate(sorted(df['user_id'].unique()))}
    item_mapping = {old: new for new, old in enumerate(sorted(df['item_id'].unique()))}
    df['user_id'] = df['user_id'].map(user_mapping)
    df['item_id'] = df['item_id'].map(item_mapping)
...
def load_amazon_digital_music() -> Tuple[pd.DataFrame, np.ndarray]:
    ...
    # FIX: drop rows where mapping failed (NaN)
...
def run_experiment_with_filters(
    ...
    for fold_idx, (train_index, test_index) in enumerate(kf.split(df)):
        ...
        # Create simple weighted ensemble baseline + uniform average baseline
        simple_ensemble = SimplePerformanceWeightedEnsemble(model_performances, verbose=True)
        uniform_ensemble = UniformAverageEnsemble(list(trained_models.keys()))

        # Ensemble evaluation
        print("   Evaluating ensembles...")
        ensemble_predictions = []
        simple_ensemble_predictions = []
        uniform_ensemble_predictions = []
        true_ratings = []
        ...
        with torch.no_grad():
            for _, row in test_df.iterrows():
                ...
                if u < train_matrix.shape[0] and i < train_matrix.shape[1]:
                    ...
                    # Uniform average ensemble prediction (explicit baseline per R#4.16)
                    uniform_pred = uniform_ensemble.predict(base_preds_cpu)
                    uniform_ensemble_predictions.append(uniform_pred)
        ...
        if ensemble_predictions:
            ...
            # Uniform average ensemble results (R#4.16 — explicit simple-average baseline)
            if uniform_ensemble_predictions:
                uniform_rmse = np.sqrt(mean_squared_error(true_ratings, uniform_ensemble_predictions))
                uniform_mae = mean_absolute_error(true_ratings, uniform_ensemble_predictions)
                fold_results['uniform_ensemble_rmse'] = uniform_rmse
                fold_results['uniform_ensemble_mae'] = uniform_mae
...
def run_filter_ablation_study(n_folds: int = 2, dataset: str = 'ml-100k'):
    ...
    # ============================================================
    # AEEMU Filter Taxonomy — addresses Reviewer #4 points 7 & 18
    # ──────────────────────────────────────────────────────────────
    # The filter bank contains 12 techniques in THREE stages:
    #
    # ┌─ STAGE 1 (pre-meta-network, GROUP A): 3 signal filters ───────────────
    # │  Applied to base-model embedding vectors before the meta-network.
    # │  They reduce high-frequency noise in the learned space.
    # │  1. Wavelet denoising    (Daubechies-4, 3 levels)
    # │  2. Spectral low-pass    (relative fc=0.1; signal = ordered prediction
    # │                           sequence for user u; "Hz" is relative to the
    # │                           sampling rate 1/interaction — R#4.5 addressed)
    # │  3. Bilateral filter     (neighbourhood = adjacent interaction steps for
    # │                           user u; σ_s=1.0, σ_r=0.1 — R#4.6 addressed)
    # │
    # ├─ STAGE 2 (post-meta-network weights, GROUP B+C): 9 signal filters ─────
    # │  Applied to the scalar ensemble prediction r̂_{ui}.
    # │  t = interaction-order index for user u (chronological, 1-indexed).
    # │  GROUP B — weight-space filters (2):
    # │  4. Kalman smoother       (Q=0.01, R calibrated on val residuals)
    # │  5. Adaptive LMS          (window=100, μ=0.1)
    # │  GROUP C — prediction-space filters (7):
    # │  6. EMA                   (α=0.3)
    # │  7. Median                (window=5)
    # │  8. Savitzky-Golay        (order=3, window=11)
    # │  9. Particle              (100 particles, σ=0.01)
    # │  10. Confidence-weighted  (variance threshold=0.8; uses MC-Dropout σ²)
    # │  11. Consensus            (variance-gated fallback to best base model)
    # │  12. Ridge Stacking       (α=1.0, trained on the same validation fold;
    # │                            linear meta-learner on base predictions only —
    # │                            distinct from the nonlinear meta-network)
    # │
    # └─ STAGE 3 (ensemble integration, GROUP D): subset from Stage 2 items 10-12
    #    The taxonomy uses items 10-12 as "ensemble-integration methods" since they
    #    combine across K base-model predictions rather than a temporal sequence.
    #
    # Ablation naming:
    #   'all_filters'   = all 12 techniques simultaneously
    #   'full_pipeline' = same as all_filters (kept for backward-compat)
    #   'best_three'    = Kalman + Adaptive LMS + EMA (empirically best trio)
    #   'no_filters'    = AEEMU meta-network only, zero signal processing
    # ============================================================
    ...
    filter_configs = {
        ...
        # ── Stage 2: Signal-Processing Filters (9 filters, R#4.7) ────────────
        # Applied sequentially to the scalar ensemble prediction r̂_ui:
        #   t = interaction-order index for user u (chronological, 1-indexed).
        # This defines the "temporal signal" for Kalman/EMA/LMS/SavGol/Particle.
        # Kalman: Q (process noise) and R (observation noise) calibrated via
        #   residual variance on validation data (addresses R#4.11).
        'kalman_only':     {'kalman': True},
        'wavelet_only':    {'wavelet': True},
        'spectral_only':   {'spectral': True},
        'adaptive_only':   {'adaptive': True},   # LMS adaptive filter
        'ema_only':        {'ema': True},
        'median_only':     {'median': True},
        'bilateral_only':  {'bilateral': True},  # neighbourhood = adjacent interaction steps
        'savgol_only':     {'savgol': True},
        'particle_only':   {'particle': True},

        # ── Stage 3: Ensemble-Integration Methods (3 methods, R#4.7) ─────────
        # These operate across base-model predictions, not temporal sequences.
        'confidence_only': {'confidence': True}, # uncertainty-weighted combination
        'consensus_only':  {'consensus': True},  # variance-gated fallback to best model
        'stacking_only':   {'stacking': True},   # linear meta-learner on base predictions
        ...
        # ── All 9 signal-processing filters (no ensemble methods) ─────────────
        # Labelled 'all_filters' to match historical JSON keys.
        'all_filters': {
            'kalman': True, 'wavelet': True, 'spectral': True,
        ...
        # ── Full pipeline: all 9 signal filters + all 3 ensemble methods ──────
        'full_pipeline': {
            'kalman': True, 'wavelet': True, 'spectral': True,
            'adaptive': True, 'ema': True, 'median': True,
            'bilateral': True, 'savgol': True, 'particle': True,
    ...
    for config_idx, (config_name, filters) in enumerate(filter_configs.items(), 1):
        ...
        # Fresh model instances for each config to prevent warm-start contamination
        # across configurations (models share same instances otherwise, since
        # PyTorch nn.Module does NOT reinitialize weights across fit() calls).
        fresh_models = {
            'NCF': BayesianNCF(n_users=n_users, n_items=n_items),
            'SASRec': SASRec(n_users=n_users, n_items=n_items),
            'LightGCN': LightGCN(n_users=n_users, n_items=n_items),
...
def compute_statistical_significance(all_results, baseline_key='no_filters'):
    ...
    for config_name, config_results in all_results.items():
        ...
        if (baseline_fold_rmses is not None and config_fold_rmses is not None and
            ...
            # Wilcoxon signed-rank test (one-sided: H1 = config RMSE > baseline RMSE, i.e. AEEMU is better)
            # zero_method='zsplit' avoids p=0.5 artefact when tied differences exist.
            try:
                diffs = config_arr - baseline_arr  # positive → config is worse than baseline
                if np.any(diffs != 0):
                    _, w_pvalue = wilcoxon(diffs, alternative='greater', zero_method='zsplit')
                else:
                    w_pvalue = 1.0
            except Exception:
                w_pvalue = 1.0
...
def generate_latex_significance_table(stat_results, top_n=10):
    ...
    for config_name, results in sorted_configs:
        ...
        # Significance marker as superscript (e.g. 0.3522$^{**}$); plain n.s. omitted for readability
        if sig and sig != 'n.s.':
            rmse_cell = f"{rmse:.4f}$^{{{sig}}}$"
        else:
            rmse_cell = f"{rmse:.4f}"

        latex.append(f"{config_latex} & {rmse_cell} & {improvement:+.2f}\\% & {t_p_str} & {w_p_str} \\\\")
